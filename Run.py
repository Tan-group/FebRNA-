...
def Combination(input_path, filex_path, filey_path, node_name, n, x_switch=0, y_switch=0):
    ...
    """
    if x_switch == 0 and ('loop' in filex_path):
        x_switch = 3
    if y_switch == 0 and ('loop' in filey_path):
        y_switch = 3
    num = 0
    ...
    with open(filey_path, 'r') as f:
        ...
        for i in range(len(lines)):
            if int(lines[i][22:26]) == left_num + 1:
                left_index = i
            if int(lines[i][22:26]) == right_num + 1:
                right_index = i
        
    #print(string, left_num, right_num, left_index, filey_path)
...
def Begin_clear():
    ...
    for name in os.listdir():
        if name not in ['database_frag','frag_e.py','Untitled.ipynb','database_seq','RNA_assessment','RNA_normalizer','example.ipynb', 'cgRNAfrag', 'fragment', 'reconstruction', 'database', 'cgRNASP-Feb', 'data', 'move_tool.py', 'Num_to_tree.py', 'parse_mid.py', 'Run.py', 'cgRNASP-Feb.c', '__pycache__', 'reconstruction.c', 'BackAA.py', 'Dot_to_num.py', 'readme.txt', 'Select_structure.py','test.ipynb','test']:
            try:
                shutil.rmtree(name)
...
def Assembly_func(input_path_f, sequence_txt_f, txt_f, path_f, fix_bp_switch):
    ...
    """
    tn = input_path_f.split('/')[1].split('_')[0]
    fraglist = []
                ...
                if type_x in ['hairpin_loop', 'hybrid_hairpin_loop']:
                    ...
                    for m in range(2):
                        if m == 0:
                            ...
                            # 如果最终还是没有结构，就返回无模板。
                            if n == 0:
                                print(f"Essemble template :{type_x}->{long_x}")
                                frag_path = path_f + fr'_%s/{temp_num}_bp/{type_x}/{long_x}/'%(tn)
                                junction_e(long_x,temp_num,frag_path,tn)
                                eindex = 1
                                n =  temp_num
    ...
    ss.Select_structure(input_path_f)
...
def seq_sample(seq, imf_seq, rest_num, bpn):
    looptype = imf_seq[0]
    loopcd = imf_seq[1].split('_')
    loopimf = imf_seq[2]
    loopseq = ''
    flist = []
    seqdict = {}
    ...
    samplelist = []
    samplelist_f = []
    for l in range(len(loopcd)):
        li1 = loopimf[2*l] -1
        li2 = loopimf[2*l+1] -1
        loopseq += seq[li1:li2]
    fpath = './database_seq/%s_bp/%s/%s/'%(bpn,looptype,imf_seq[1])
    seqf = fpath + 'seqinfo.txt'
    with open(seqf) as f:
        line = f.readlines()
    f.close()

    for l in line:
        flist.append(l.split('  ')[0].strip())
        seqdict[l.split('  ')[0].strip()] = l.split('  ')[1].strip() 
    looplen = len(seqdict[flist[0]]) 
    if looplen >=1:
        if len(flist) > 1:
            for ff in flist:
                fseq = seqdict[ff]
                sd = seqalign(loopseq,fseq)
                sddict[ff] = sd
    sdsort = sorted(sddict.items(), key = lambda kv:(kv[1], kv[0]), reverse = True)
    #print(seqf,loopseq, sdsort[0:3])
    #for sl in range(rest_num):
    #    samplelist.append(fpath + sdsort[sl][0])
    sdm = sdsort[0][1]
    for sl in range(len(flist)):
        if  sdsort[sl][1] == sdm:
            samplelist.append(fpath + sdsort[sl][0])
        else:
            break
    if len(samplelist) > rest_num:
        samplelist_f = random.sample(samplelist, 2)
    else:
        samplelist_f = samplelist
    #print(fpath, loopseq,samplelist)
                    
    return samplelist_f  


def Combination_seq(input_path, filex_path, filey_path, node_name, n, node, d_stems, ssenum, txt_f, seq, x_switch=0, y_switch=0,):
    """
    input_path 彻底把每一次拼装都分开
    根据提供的A片段的地址和B片段地址,node_name,接口bp数n 来拼装
    并得到以node_name(Node(n))命名的文件夹，里面装着这一步完成后所有拼装出来的结构以{num++}.pdb命名
    ...
    if x_switch == 0 and ('loop' in filex_path):
        x_switch = 3
        #fs = filex_path.split('database_seq')
        #filex_path = fs[0] + 'database_seq' +fs[1]
        #print(filex_path)
    if y_switch == 0 and ('loop' in filey_path):
        y_switch = 3
        #fs = filey_path.split('database_seq')
        #filey_path = fs[0] + 'database_seq' +fs[1]
        #print(filey_path)
    num = 0
    x_list = glob.glob(filex_path)
    ...
    if "hybrid" in filex_path:
        ...
        if x_switch > len(x_list):
            ...
            if rest_num > len(x_change_list):
                rest_num = len(x_change_list)
            #if 'loop' in filex_path:
            star_mark, stop_mark = e_mark(txt_f)
            imf_seq = Bref_Frag_index(node, d_stems, ssenum, n, star_mark, stop_mark)
            fsample = seq_sample(seq, imf_seq, rest_num, n)
            x_list += fsample
            #else:
            #   x_list += random.sample(glob.glob(filex_path), rest_num)
    else:
        ...
        if x_switch != 0 and len(x_list) > x_switch:
            ...
            star_mark, stop_mark = e_mark(txt_f)
            imf_seq = Bref_Frag_index(node, d_stems, ssenum, n, star_mark, stop_mark)
            x_list = seq_sample(seq, imf_seq, x_switch, n)
            #else:
            #    x_list = random.sample(glob.glob(filex_path), x_switch)
    ...
    if "hybrid" in filey_path:
        ...
        if y_switch != 0 and len(y_list) > y_switch:
            ...
            star_mark, stop_mark = e_mark(txt_f)
            imf_seq = Bref_Frag_index(node, d_stems, ssenum, n, star_mark, stop_mark)
            fsample = seq_sample(seq, imf_seq, y_switch, n)
            y_list = fsample
            #else:
                #y_list = random.sample(glob.glob(filey_path), y_switch)
    #if 'loop' in filex_path:
    #    print(filex_path, x_list,  end= '\n')
    #if 'loop' in filey_path:
    #    print(filey_path, y_list,end= '\n')
    # 这里的循环xy可以换一下位置，因为Y一般少一点
    #print(x_list,y_list,filey_path)
    for y in y_list:
        for x in x_list:
            num += 1
            # 构建文件夹和文件路径
...
def Combination_beside_seq(input_path, filex_path, filey_path, beside_name, insect, long, flag, x_switch=0, y_switch=0):
    """
    def func_beside(X, Y, insect, long, flag=0):
    根据提供的A片段的地址和B片段地址,node_name,接口bp数n 来拼装
    并得到以node_name(Node(n))命名的文件夹，里面装着这一步完成后所有拼装出来的结构以{num++}.pdb命名
...
def Assembly_func_seq(input_path_f, sequence_txt_f, txt_f, path_f, fix_bp_switch):
    """
    拼装函数
    :param sequence_txt_f: 二级结构序列形式
    :param input_path_f 存放拼装结构
        ...
        for iii in order_lt:
            ...
            if True:
                if type_x in ['hairpin_loop', 'hybrid_hairpin_loop']:
                    ...
                    imf_seq = Bref_Frag_index(iii, d_stems, ii[4], n, star_mark, stop_mark)
                    #print(imf_seq)
                    Combination_seq(input_path_f, file_x_path, file_y_path, dir_name, n, \
                                    iii, d_stems, ii[4], txt_f, sequence_txt_f, x_boot, y_boot,)
...
if __name__ == '__main__':
    ...
    # todo 建并行文件夹
    for i_begin in range(1, num_flag):
        dir_path_begin = rf"./{i_begin}_foils"
        os.mkdir(dir_path_begin)

    # todo 多进程拼装，同时把文件整合 和 更新序列 整理放在了拼装程序里。
    if num_flag == 6:
        p0 = multiprocessing.Process(target=Assembly_func_seq, args=(fr"./1_foils", sequence_txt, txt, pathseq, flag))
        p1 = multiprocessing.Process(target=Assembly_func_seq, args=(fr"./2_foils", sequence_txt, txt, pathseq, flag))
        p2 = multiprocessing.Process(target=Assembly_func_seq, args=(fr"./3_foils", sequence_txt, txt, pathseq, flag))
    ...
    energyl = []
    for e in glob.glob('./*_foils_Energy_out.csv'):
        findex = e.split('/')[1].split('_')[0]
        with open(e, 'r') as f:
            num = 1
            lines = f.readlines()
        for l in lines:
            energyl.append(findex+'_'+l)
        f.close()
    energyl.sort(key=lambda x: float(x.split(',')[1].strip()))
    for jjj in energyl[:select_num]:
        fi = jjj.split('_')[0]
        pi = jjj.split('_')[1]
        old_path = fr"./%s_foils/FINAL_Result/{pi.split(',')[0]}"%(fi)
